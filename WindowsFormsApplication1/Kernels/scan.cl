...
void scanExclusiveLocal2(
	...
	{
		W = (4 * WORKGROUP_SIZE - 1) + (4 * WORKGROUP_SIZE) * get_global_id(0);
		data = d_Dst[W] + d_Src[W];
	}
    //Compute
...
#define scan_max 262144

__kernel __attribute__((reqd_work_group_size(WORKGROUP_SIZE, 1, 1)))
void scanCorrect(
    __global uint *d_Dst,
    __global const uint *d_Src,
	__global const uint *d_SrcIn
){
	int i, W, M;

    uint data = d_Src[get_global_id(0)];
	M = get_global_id(0)/scan_max;
	for (i=1; i<=M; i++)
	{
		W = scan_max*i-1;
		data += d_Src[W] + d_SrcIn[W];
	}
	d_Dst[get_global_id(0)] = data;
}