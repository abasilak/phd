...
uniform vec4  color_background;
	...
	vec4 resolveTrim(const int num)
	{
		...
		{
			vec4 C = unpackUnorm4x8(floatBitsToUint(maxC.r));
	...
	vec4 resolveTrim(const int num)
	{
		...
		for(int i=0; i<num; i++)
		{
			...
			if(Ci.g <= capping)
			{
				...
				if(useTransparency)
				{
					c =	unpackUnorm4x8(floatBitsToUint(Ci.r));
					color = vec4(c.rgba);//, transparency);
					if(correctAlpha)
					{
						if(i%2 == num%2)
						{
	...
	vec4 resolveLayer(const int num, const bool sorted)
	{
		if(!sorted)
			sort(num);
		return unpackUnorm4x8(floatBitsToUint(fragments[layer].r));
	}
	...
	vec4 resolveCoplanarity(const int num, const bool sorted)
	{
		...
		while (i<num)
		{
			Ci = fragments[i];
			if(i==layer)
				C0 = unpackUnorm4x8(floatBitsToUint(Ci.r));
...
//			if(Ci.g == 1.0f)
	//			break;
			...
			//if(i==0)
			//{
				//C0 = unpackUnorm4x8(floatBitsToUint(fragments[i].r));
				//if (correctAlpha) thicknessAlpha = fragments[i].g*0.5f;
			//}

			//if(useTranslucency)
			//{
			//	if(i%2==1) thickness += fragments[i].g - prevZ;
			//	prevZ = Ci.g;
			//}
			//else
			//	if(useTransparency)
			//{
				//color = vec4(unpackUnorm4x8(floatBitsToUint(fragments[i].r)));
				//if(correctAlpha)
				//{
				//	if(i%2 == num%2)
					//{
		...
		for (int i = 0; i<num && fragments[i].g < 1.0f; i++)
		{
			color = vec4(unpackUnorm4x8(floatBitsToUint(fragments[i].r)));
			finalColor += color*(1.0f - finalColor.a);
		}
	...
	{
		if(sorted)
			return unpackUnorm4x8(floatBitsToUint(fragments[0].r));
		else
		{
			vec2 Ci, maxC = vec2(0.0f,1.0f);
			...
			return unpackUnorm4x8(floatBitsToUint(maxC.r));
		}
	}
#endif
