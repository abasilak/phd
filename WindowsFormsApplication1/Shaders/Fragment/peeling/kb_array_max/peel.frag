...
#extension GL_NV_fragment_shader_interlock : enable
	...
	//layout(binding = 6, r32ui) coherent uniform uimage2DRect  image_skata;
	...
	//uint  addPixelFragCounter	(				)					 {return  imageAtomicAdd (image_skata, ivec2(gl_FragCoord.xy), 1U);}
	...
	void insertFragmentArrayMax(float C)
	{
		...
		else
		{
			...
			//}
#else
			id = setMaxFromGlobalArray(); // not correct - use it for comparison reasons
#endif
		}
...
#if early_clipping
		if(gl_FragCoord.z >= getPixelFragValue(0).g) // faster - [clipping is faster]
			discard;
#endif
		{
