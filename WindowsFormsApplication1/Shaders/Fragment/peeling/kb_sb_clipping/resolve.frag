...
#include "resolve.h"
	...
	layout(binding = 4, r32ui) readonly uniform uimage2DArray image_count_next_sem;
	layout(binding = 5, rg32f) readonly uniform  imageBuffer  image_peel;

	uint getPixelFragCounter(					) {return imageLoad (image_count_next_sem, ivec3(gl_FragCoord.xy,0)).x;}
	uint getPixelNextAddress(					) {return imageLoad (image_count_next_sem, ivec3(gl_FragCoord.xy,1)).x;}
